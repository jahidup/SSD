...
if (process.env.GEMINI_API_KEY) {
  try {
    ...
    // For @google/generative-ai npm package, the class is GoogleGenAI or we use standard package initialization.
    // The standard official import is usually: const { GoogleGenAI } = require('@google/generative-ai'); 
    // or standard newer version initialization.
    // Let's write a bulletproof initialization.
    // If the official SDK is standard GoogleGenAI, we do: new GoogleGenAI({ apiKey: ... })
    // If it's the legacy, we use: GoogleGenAI.
    // Let's wrap GoogleGenAI initialization safely.
  } catch (e) {
